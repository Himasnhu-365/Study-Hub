...
# --- 3. SESSION STATE INITIALIZATION (Data Save Rakhne Ke Liye) ---
...
if st.button("🚀 IGNITE GENERATOR", use_container_width=True):
    if not api_key:
        st.error("⚠️ System Alert: Please enter your API Key in the sidebar.")
    elif not topic:
        ...
        try:
            ...
            # Notes & Formula Logic with Download Button
                ...
                with st.status("⚡ Writing Smart Notes...", expanded=True) as status:
                    ...
                    # Notes print karna aur variable me save karna
                ...
                st.write("") 
                st.download_button(
                    label="📥 Download & Save Notes (Offline)",
                    data=full_notes,
                    file_name=f"{exam_target}_{topic}_Notes.md",
                    mime="text/markdown",
